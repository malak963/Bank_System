...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">{{ __('Card Details') }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ __('Card Information') }}</h2>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm text-gray-500">{{ __('Card Number') }}</p>
                    ...
                    <div>
                        <p class="text-sm text-gray-500">{{ __('Account') }}</p>
                    ...
                    <div>
                        <p class="text-sm text-gray-500">{{ __('Issued') }}</p>
                        <p class="text-lg font-medium text-gray-900">{{ $card->issued_at?->format('M d, Y') ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">{{ __('Expires') }}</p>
                ...
                @if($card->blocked_at)
                    <div class="mt-4 bg-red-50 border border-red-200 rounded-lg p-4">
                        <h3 class="text-sm font-semibold text-red-800">{{ __('Block Information') }}</h3>
                ...
                @if($card->replacement_reason)
                    <div class="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <h3 class="text-sm font-semibold text-yellow-800">{{ __('Replacement Information') }}</h3>
        ...
        <div>
                ...
                <div class="space-y-3">
                    ...
                    @if($card->isActive())
                        <form action="{{ route('cards.block', $card) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Block Reason') }}</label>
                            ...
                            <button type="submit" class="w-full bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 text-sm">{{ __('Block Card') }}</button>
                        </form>
                    @endif
                    ...
                    @if($card->status === \App\Modules\Cards\Enums\CardStatus::Blocked)
                        <form action="{{ route('cards.unblock', $card) }}" method="POST">
                            @csrf
                            @method('POST')
                            <button type="submit" class="w-full bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 text-sm">{{ __('Unblock Card') }}</button>
                        </form>
                    @endif
                    ...
                    @if($card->status->requiresReplacement())
                        <form action="{{ route('cards.replace', $card) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Replacement Reason') }}</label>
                                <textarea name="reason" required rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" placeholder="{{ __('Enter reason...') }}"></textarea>
                            </div>
                            <button type="submit" class="w-full bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 text-sm">{{ __('Replace Card') }}</button>
                        </form>
                    @endif
                </div>
            </div>
            ...
            <!-- Limits & Features -->
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('Limits & Features') }}</h3>
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">{{ __('Daily Limit') }}</span>
                        <span class="text-sm font-medium">${{ number_format($card->daily_limit, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">{{ __('Monthly Limit') }}</span>
            ...
            @if($card->isActive())
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('PIN Management') }}</h3>
                    <form action="{{ route('cards.update-pin', $card) }}" method="POST">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('New PIN') }}</label>
                            ...
                            <button type="submit" class="w-full bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 text-sm">{{ __('Update PIN') }}</button>
                        </div>
                    </form>
                </div>
            @endif
