    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        @if($criticalEvents->count() > 0)
            <div class="bg-red-50 border border-red-200 rounded-lg p-6">
                ...
                <a href="#" class="text-red-600 hover:text-red-800 text-sm">View All →</a>
            </div>
        @endif
        ...
        @if($fraudAlerts->count() > 0)
            <div class="bg-orange-50 border border-orange-200 rounded-lg p-6">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-orange-800">Fraud Alerts</h3>
                </div>
                <p class="text-orange-700 mb-2">{{ $fraudAlerts->count() }} fraud alerts requiring attention</p>
                <a href="#" class="text-orange-600 hover:text-orange-800 text-sm">View All →</a>
            </div>
        @endif
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">User/Customer</th>
                ...
                @forelse($events as $event)
                    <tr class="hover:bg-gray-50 {{ $event->isCritical() ? 'bg-red-50' : '' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('security.show', $event) }}" class="text-emerald-600 hover:text-emerald-900">View</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">No security events found</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
