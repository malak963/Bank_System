...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Security Event Details</h1>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex justify-between items-start mb-6">
                    ...
                    @switch($event->security_level->value)
                        @case('low')
                            <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-green-100 text-green-800">Low</span>
                            @break
                        @case('medium')
                            <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Medium</span>
                            @break
                        @case('high')
                            <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-orange-100 text-orange-800">High</span>
                            @break
                        @case('critical')
                            <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-red-100 text-red-800">Critical</span>
                            @break
                    @endswitch
                </div>
                ...
                @if($event->resolution_notes)
                    <div class="mt-4 bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                        <h3 class="text-sm font-semibold text-emerald-800">Resolution Notes</h3>
                ...
                <div class="space-y-3">
                    @if(!$event->is_resolved)
                        <form action="{{ route('security.resolve', $event) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="resolution_notes" required rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" placeholder="Enter resolution notes..."></textarea>
                            </div>
                            <button type="submit" class="w-full bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 text-sm">Resolve Event</button>
                        </form>
                    @endif
                    ...
                    @if(!$event->blocked)
                        <form action="{{ route('security.block', $event) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="block_duration_hours" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" placeholder="Leave empty for permanent">
                            </div>
                    ...
                    @else
                        <form action="{{ route('security.unblock', $event) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 text-sm">Unblock User</button>
                        </form>
                    @endif
                </div>
            </div>
